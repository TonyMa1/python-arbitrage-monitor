...
class OrderbookSpreadMonitor(SpreadMonitorBase):
    ...
    async def monitor(self, exchange: ccxtpro.Exchange, index: str, symbols: List[str]):
        ...
        while self.running:
            try:
                orderbooks = await exchange.watch_order_book_for_symbols(symbols, limit=limit)
                await asyncio.gather(*[
                    self.process_order_book(orderbook, index, self.latencies[exchange_name].get("time_diff", 0))
    ...
    async def process_order_book(self, order_book: Dict, index: str, time_diff: float):
        ...
        pair_names = self.symbol_map[index][symbol]["pair_names"]
        for pair_name in pair_names:
            # Use setdefault for initialization and update
            data = self.pair_data.setdefault(pair_name, {
                "pair_name": pair_name,
                "spread_pct": 0.0,
                "buy_a_sell_b_spread_pct": 0.0,
                "buy_b_sell_a_spread_pct": 0.0,
                "bid_price_a": 0.0,
                "bid_volume_a": 0.0,
                "ask_price_a": 0.0,
                "ask_volume_a": 0.0,
            ...
            if order_book["bids"]:
                data[f"bid_price_{index}"], data[f"bid_volume_{index}"] = order_book["bids"][0]
            if order_book["asks"]:
                data[f"ask_price_{index}"], data[f"ask_volume_{index}"] = order_book["asks"][0]

            data[f"elapsed_time_{index}"] = time.time() * 1e3 - (order_book["timestamp"] + time_diff)
            await self.calculate_spread(pair_name)


    async def calculate_spread(self, pair_name: str):
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(self._executor, self._calculate_orderbook_spread_sync, pair_name)

    def _calculate_orderbook_spread_sync(self, pair_name:str):
        data = self.pair_data[pair_name]
        try:
            if all([data["ask_price_a"], data["bid_price_a"], data["ask_price_b"], data["bid_price_b"]]):
                buy_b_sell_a_spread = data["bid_price_a"] - data["ask_price_b"]
                buy_a_sell_b_spread = data["bid_price_b"] - data["ask_price_a"]

                data["buy_b_sell_a_spread"] = buy_b_sell_a_spread
                data["buy_b_sell_a_spread_pct"] = buy_b_sell_a_spread / data["ask_price_b"] if data["ask_price_b"]!=0 else 0 # avoid ZeroDivisionError
                data["buy_a_sell_b_spread"] = buy_a_sell_b_spread
                data["buy_a_sell_b_spread_pct"] = buy_a_sell_b_spread / data["ask_price_a"] if data["ask_price_a"]!= 0 else 0 # avoid ZeroDivisionError
                data["spread_pct"] = max(data["buy_b_sell_a_spread_pct"], data["buy_a_sell_b_spread_pct"])

        except (TypeError) as e:
             print(f"Calculate spread error for {pair_name}: {e}")